...
<!-- Style format for deleteUserConfirm and the navbar  -->
<style>
            body {
...
<!-- This navigation bar displays all the pages in the website --> 
	...
	<%
		...
		User us = DBUtil.getUserById(userId);
	
		out.print("You will delete user with ID: " + userId);

		boolean didDelete = DBUtil.deleteUser(userId);
		...
		if (didDelete){
			...
			out.print("<h2>Id " + us.getUserId() + " does not exist! "+ "</h2>");
		}
		else {
			out.print("<h2>Error deleting user, try again</h2>");
		}
	%>
	
	<h1>Thank you <%=us.getUserId() %> DELETED! </h1>
